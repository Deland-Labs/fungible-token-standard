...
Feature: token transfer
  ...
  Scenario Outline:Transfer token to a receiver
    ...
    And Check that the transfer fees of <token> by <diff> charged are correct
    Examples:
      | userA     | userB        | diff | token        | amountA  | amountB |
      | dft_miner | dft_receiver | 100  | dft_basic    | 99899.99 | 100     |
      | dft_user1 | dft_receiver | 100  | dft_basic2   | 99899.99 | 100     |
      | dft_user2 | dft_receiver | 100  | dft_burnable | 99898    | 100     |
      | dft_user3 | dft_receiver | 100  | dft_mintable | 99890    | 100     |